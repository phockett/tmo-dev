...
# **********************************************************************
    ...
    def genCorrVMIXmulti(self, filterOptions={}, **kwargs):
        """Wrapper for genCorrVMIX with multiple filter sets."""
        ...
        for item in self.filters.keys():
            if self.verbose['main']:
                print(f'Generating covariance VMI images for filters: {item}')
            ...
            self.genCorrVMIX(name=item, filterOptions = self.filters[item], **kwargs)
